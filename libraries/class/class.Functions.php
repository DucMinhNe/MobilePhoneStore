...
'script' => '@<script[^>]*?>.*?</script>@si',
    ...
    {
        if (preg_match('/^(\d{4})(\d{2})(\d{2})(\d{2})$/', $number, $matches) || preg_match('/^(\d{3})(\d{4})(\d{4})$/', $number, $matches)) {
            return $matches[1] . $dash . $matches[2] . $dash . $matches[3] . $dash . $matches[4];
        }
    }
    ...
    public function getSize($id = 0)
    {
        ...
        $str = '<select id="dataSize" name="dataSize[]" class="select multiselect multiselect-size" multiple="multiple" >';
