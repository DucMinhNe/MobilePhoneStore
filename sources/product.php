...
if ($id != '') {
    /* Lấy sản phẩm detail */
    $rowDetail = $d->rawQueryOne("select type, id, dungluong$lang, name$lang, thongso$lang, slugvi, link_youtube, slugen, desc$lang, content$lang, code, view, id_brand, id_list, id_cat, id_item, id_sub, photo, options, discount, sale_price, regular_price, option_size from #_product where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));
    
    $tragop = $cache->get("select name$lang, photo, link from #_photo where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('tra-gop'), 'result', 7200);

    $sanphamkhac = $cache->get("select name$lang, khuyenmai$lang, baohanh$lang, hotro$lang from #_static where type = ? limit 0,1", array('san-pham-ngoai-apple'), 'fetch', 7200);
    ...
    $listSize = (!empty($rowDetail["option_size"])) ? json_decode($rowDetail["option_size"], true) : null;
     //$func->dump($listSize);
    ...
    if (!empty($productSize)) {
        $rowSize = $d->rawQuery("select id, name$lang from #_size where type='" . $type . "' and id in ($productSize) order by numb, id desc");
    }

    /* Lấy cấp 1 */
    $productList = $d->rawQueryOne("select id, name$lang, khuyenmai$lang, baohanh$lang, hotro$lang, desc$lang, slugvi, slugen from #_product_list where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($rowDetail['id_list'], $type));

    //$func->dump($productList, true);
    ...
    $where2 = "";
    $where2 = "id <> ? and id_item = ? and type = ? and find_in_set('hienthi',status)";
    $params2 = array($id, $rowDetail['id_item'], $type);

    $sql2 = "select photo, name$lang, desc$lang, dungluong$lang, slugvi, slugen, sale_price, regular_price, discount, id from #_product where $where2 order by numb,id desc $limit";
    $product2 = $d->rawQuery($sql2, $params2);
    
    $curPage = $getPage;
    $perPage = 5;
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select photo, name$lang, desc$lang, slugvi, slugen, giadukien, sale_price, regular_price, discount, gialendoi, id from #_product where $where order by numb,id desc $limit";
    ...
    $where3 = "";
    $where3 = "id <> ? and id_list = ? and type = ? and find_in_set('hienthi',status)";
    $params3 = array($id, $rowDetail['id_list'], $type);

    $curPage3 = $getPage3;
    $perPage3 = 6;
    $startpoint3 = ($curPage3 * $perPage3) - $perPage3;
    $limit3 = "limit 0,6";
    $sql3 = "select photo, name$lang, desc$lang, slugvi, slugen, giadukien, sale_price, regular_price, discount, gialendoi, id from #_product where $where3 order by numb,id desc $limit3";
    $product3 = $d->rawQuery($sql3, $params3);
    $sqlNum3 = "select count(*) as 'num' from #_product where $where3 order by numb,id desc";
    $count3 = $d->rawQueryOne($sqlNum3, $params3);
    $total3 = (!empty($count3)) ? $count3['num'] : 0;
    $url3 = $func->getCurrentPageURL();
    $paging3 = $func->pagination($total3, $perPage3, $curPage3, $url3);
    ...
    /* Lấy cấp 1 detail */
    $productList = $d->rawQueryOne("select id, name$lang, desc$lang, slugvi, slugen, type, photo, options from #_product_list where id = ? and type = ? limit 0,1", array($idl, $type));
    ...
    /* Lấy cấp 1 */
    $productList = $d->rawQueryOne("select id, name$lang, desc$lang, slugvi, slugen from #_product_list where id = ? and type = ? limit 0,1", array($productCat['id_list'], $type));
    ...
    /* Lấy cấp 1 */
    $productList = $d->rawQueryOne("select id, name$lang, desc$lang, slugvi, slugen from #_product_list where id = ? and type = ? limit 0,1", array($productItem['id_list'], $type));
    ...
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select photo, name$lang, slugvi, slugen, sale_price, regular_price, discount, gialendoi, id from #_product where $where order by numb,id desc $limit";
    $product = $d->rawQuery($sql, $params);
    $sqlNum = "select count(*) as 'num' from #_product where $where order by numb,id desc";
    $count = $d->rawQueryOne($sqlNum, $params);
    $total = (!empty($count)) ? $count['num'] : 0;
    ...
    /* Lấy cấp 1 */
    $productList = $d->rawQueryOne("select id, name$lang, desc$lang, slugvi, slugen from #_product_list where id = ? and type = ? limit 0,1", array($productSub['id_list'], $type));
    ...
    if (isset($keothom) && $keothom == true) {
        $where .= " and find_in_set('keothom', status)";
    }
    $params = array($type);
    ...
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select photo, name$lang, desc$lang, id_list, slugvi, slugen, giadukien, sale_price, regular_price, discount, gialendoi, id from #_product where $where order by numb,id desc $limit";
    $product = $d->rawQuery($sql, $params);
    $sqlNum = "select count(*) as 'num' from #_product where $where order by numb,id desc";
    $count = $d->rawQueryOne($sqlNum, $params);
    $total = (!empty($count)) ? $count['num'] : 0;
