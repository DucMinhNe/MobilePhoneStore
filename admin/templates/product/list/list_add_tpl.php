        ...
        <div class="row">
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if (isset($config['product'][$type]['khuyenmai_list']) && $config['product'][$type]['khuyenmai_list'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="khuyenmai<?= $k ?>">Khuyến mãi (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['product'][$type]['khuyenmai_cke_list']) && $config['product'][$type]['khuyenmai_cke_list'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[khuyenmai<?= $k ?>]" id="khuyenmai<?= $k ?>" rows="5" placeholder="Khuyến mãi (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('khuyenmai' . $k)) ?: $func->decodeHtmlChars(@$item['khuyenmai' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if (isset($config['product'][$type]['baohanh_list']) && $config['product'][$type]['baohanh_list'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="baohanh<?= $k ?>">Bảo hành (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['product'][$type]['baohanh_cke_list']) && $config['product'][$type]['baohanh_cke_list'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[baohanh<?= $k ?>]" id="baohanh<?= $k ?>" rows="5" placeholder="Bảo hành (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('baohanh' . $k)) ?: $func->decodeHtmlChars(@$item['baohanh' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if (isset($config['product'][$type]['hotro_list']) && $config['product'][$type]['hotro_list'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="hotro<?= $k ?>">Hỗ trợ (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['product'][$type]['hotro_cke_list']) && $config['product'][$type]['hotro_cke_list'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[hotro<?= $k ?>]" id="hotro<?= $k ?>" rows="5" placeholder="Hỗ trợ (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('hotro' . $k)) ?: $func->decodeHtmlChars(@$item['hotro' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
            ...
            <div class="<?= $colRight ?>">
                ...
                <?php if (isset($config['product'][$type]['images2_list']) && $config['product'][$type]['images2_list'] == true) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
                            <h3 class="card-title">Hình ảnh <?= $config['product'][$type]['title_main_list'] ?> (2)</h3>
                            <div class="card-tools">
                                <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            $photoDetail2['dimension'] = "Width: " . $config['product'][$type]['width2_list'] . " px - Height: " . $config['product'][$type]['height2_list'] . " px (" . $config['product'][$type]['img_type_list'] . ")";
