            ...
            <div class="<?= $colLeft ?>">
                <div class="card card-primary card-outline text-sm">
                    ...
                    <div class="card-body">
                        ...
                        <?php } ?>
                        <?php if (isset($config['static'][$type]['demgio']) && $config['static'][$type]['demgio'] == true) { ?>
                            <div class="form-group">
                                <label class="d-block" for="demgio">Thời gian khuyến mãi:</label>
                                <input type="datetime-local" class="form-control text-sm" name="data[demgio]" id="demgio" placeholder="Thời gian khuyến mãi" value="<?= (!empty($flash->has('demgio'))) ? $flash->get('demgio') : @$item['demgio'] ?>">
                            </div>
                        <?php } ?>
                        <div class="form-group">
                                    ...
                                    <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                        ...
                                        <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                            <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                                ...
                                                <?php if (isset($config['static'][$type]['khuyenmai']) && $config['static'][$type]['khuyenmai'] == true) { ?>
                                                    <div class="form-group">
                                                        <label for="khuyenmai<?= $k ?>">Khuyến mãi (<?= $k ?>):</label>
                                                        <textarea class="form-control for-seo text-sm <?= (isset($config['static'][$type]['khuyenmai_cke']) && $config['static'][$type]['khuyenmai_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[khuyenmai<?= $k ?>]" id="khuyenmai<?= $k ?>" rows="5" placeholder="Khuyến mãi (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('khuyenmai' . $k)) ?: $func->decodeHtmlChars(@$item['khuyenmai' . $k]) ?></textarea>
                                                    </div>
                                                <?php } ?>

                                                <?php if (isset($config['static'][$type]['baohanh']) && $config['static'][$type]['baohanh'] == true) { ?>
                                                    <div class="form-group">
                                                        <label for="baohanh<?= $k ?>">Bảo hành (<?= $k ?>):</label>
                                                        <textarea class="form-control for-seo text-sm <?= (isset($config['static'][$type]['baohanh_cke']) && $config['static'][$type]['baohanh_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[baohanh<?= $k ?>]" id="baohanh<?= $k ?>" rows="5" placeholder="Bảo hành (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('baohanh' . $k)) ?: $func->decodeHtmlChars(@$item['baohanh' . $k]) ?></textarea>
                                                    </div>
                                                <?php } ?>

                                                <?php if (isset($config['static'][$type]['hotro']) && $config['static'][$type]['hotro'] == true) { ?>
                                                    <div class="form-group">
                                                        <label for="hotro<?= $k ?>">Hỗ trợ (<?= $k ?>):</label>
                                                        <textarea class="form-control for-seo text-sm <?= (isset($config['static'][$type]['hotro_cke']) && $config['static'][$type]['hotro_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[hotro<?= $k ?>]" id="hotro<?= $k ?>" rows="5" placeholder="Hỗ trợ (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('hotro' . $k)) ?: $func->decodeHtmlChars(@$item['hotro' . $k]) ?></textarea>
                                                    </div>
                                                <?php } ?>
