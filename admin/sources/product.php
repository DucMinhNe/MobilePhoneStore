...
function saveMan()
{
    ...
    $dataPriceSize = (!empty($_POST['dataSizePrice'])) ? $_POST['dataSizePrice'] : null;
    $option_size_hidden = (!empty($_POST['option_size_hidden'])) ? json_decode($_POST['option_size_hidden'], true) : null;


    /* Price Sale */
    foreach ($dataPriceSize as $k => $v) {

        if (!empty($_FILES['photo_price']['name'][$v["id_size"]])) {
            $dataPriceSize[$v["id_size"]]['photo_price'] = isset($_FILES['photo_price']['name'][$v["id_size"]]) ?  $_FILES['photo_price']['name'][$v["id_size"]] : '';
        } else {
            $dataPriceSize[$v["id_size"]]['photo_price'] = $option_size_hidden[$v["id_size"]]['photo_price'];
        }

        // $func->dump($dataPriceSize[$v["id_size"]]['photo_price']);

        $dataPriceSize[$v["id_size"]]['regular_price'] = (isset($dataPriceSize[$v["id_size"]]['regular_price']) &&
            $dataPriceSize[$v["id_size"]]['regular_price'] != '') ? str_replace(
            ",",
            "",
            $dataPriceSize[$v["id_size"]]['regular_price']
        ) : 0;

        // $func -> dump($dataPriceSize[$v["id_size"]]['regular_price']);

        $dataPriceSize[$v["id_size"]]['sale_price'] = (isset($dataPriceSize[$v["id_size"]]['sale_price']) &&
            $dataPriceSize[$v["id_size"]]['sale_price'] != '') ? str_replace(",", "", $dataPriceSize[$v["id_size"]]['sale_price']) :
        0;
        $dataPriceSize[$v["id_size"]]['discount'] = (isset($dataPriceSize[$v["id_size"]]['discount']) &&
            $dataPriceSize[$v["id_size"]]['discount'] > 0) ? $dataPriceSize[$v["id_size"]]['discount'] : 0;
    }
    if ($data) {
        ...
        $data['gialendoi'] = (isset($data['gialendoi']) && $data['gialendoi'] != '') ? str_replace(",", "", $data['gialendoi']) : 0;
        $data['giadukien'] = (isset($data['giadukien']) && $data['giadukien'] != '') ? str_replace(",", "", $data['giadukien']) : 0;
        $data['sale_price'] = (isset($data['sale_price']) && $data['sale_price'] != '') ? str_replace(",", "", $data['sale_price']) : 0;
        $data['tietkiem'] = (isset($data['tietkiem']) && $data['tietkiem'] != '') ? str_replace(",", "", $data['tietkiem']) : 0;
        $data['discount'] = (isset($data['discount']) && $data['discount'] != '') ? $data['discount'] : 0;
        $data['type'] = $type;
        $data['option_size'] = (!empty($dataPriceSize)) ? json_encode($dataPriceSize) : "";
    ...
    if ($id && $act != 'save_copy') {
        ...
        if ($d->update('product', $data)) {
            ...
            /* Xử Lý Sửa Hình Ảnh */
            foreach ($dataPriceSize as $k => $v) {
                $tpm_photo = $_FILES['photo_price']['tmp_name'][$v['id_size']];
                $type_photo = $_FILES['photo_price']['type'][$v['id_size']];
                $file_name1 = $_FILES['photo_price']['name'][$v['id_size']];
                move_uploaded_file($tpm_photo,  UPLOAD_PRODUCT .  $file_name1);
            }
